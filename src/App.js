...
// News Templates: {text: string, impact: % change (0.1 = +10%)}
...
function App() {
  ...
  const [newsFeed, setNewsFeed] = useState([]); // Stores active news
  ...
  const handleMarketUpdate = () => {
    // 1. Decide if News Happens (50% chance)
    let activeNews = null;
    if (Math.random() > 0.5) {
      ...
      // Add to news feed (keep max 4 items)
    ...
    setMarketWatches((current) =>
      current.map((watch) => {
        ...
        // If this watch is the subject of the news, apply the BIG impact
        if (activeNews && watch.brand === activeNews.brand) {
          changePercent = activeNews.impact;
        } else {
          // Otherwise, normal random fluctuation (+/- 5%)
